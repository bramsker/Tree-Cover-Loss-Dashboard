...
sidebar <- dashboardSidebar(
  sidebarMenu(
    ...
    menuItem('Spatial', tabName = 'spatial', icon = icon('chart-line'))
  )
)
# body
body <- dashboardBody(
  ...
  tabItems(
    ...
    tabItem(tabName = 'tabular',
            ...
            fluidRow(
              box(title = "Loss Area (Ha)",
                  tableOutput("table1"),
                  downloadButton("download", "Download")
              ),
              box(title = "Trend of Loss (Ha)",
                plotOutput("plot2")
              )
            ),
    ),
    ...
    tabItem(tabName = 'spatial',
            leafletOutput("leafplot", height = 900)
    )
  )
)
...
#----Calculate loss area based on pixel size and value each zone---------------------------------------
  list_lossarea <- lapply(list_masked, function(e) {data.frame(tapply(area(e), e[], sum))})
...
#----Calculate the loss into Ha and round it-----------------------------------------------------------
  list_lossarea <- lapply(list_lossarea, function (f) {data.frame(round(f/10000, 2))})
